...
public class ProductInfoDao {
	...
	public ArrayList<ProductInfoVo> getList(int startRow, int endRow, String category, int order, String colorVal, String sizeVal, String priceVal, String div){
		...
		try {
			...
			String colorwhere="", sizewhere="", pricewhere="";
			if(div!=null) {
			...
			String otable=" ", owhere=" ", orderby=" ", oselect=" ";
			switch(order) {
			...
			if(startRow<=endRow) {
				...
				int ecnt=getCountExist(category,order,colorVal,sizeVal,priceVal,div);
				sql="select * from "+
					"(select aa.*,rownum rnum " +
					"from (select distinct pn.pcode, cname, pname, price, color.colnum, color, savefilename, scnt " + oselect +
					...
					") where rnum between ? and ?";
				pstmt=con.prepareStatement(sql);
				pstmt.setInt(1, (startRow-ecnt));
				pstmt.setInt(2, (endRow-ecnt));
				rs=pstmt.executeQuery();
				while(rs.next()) {
					list.add(new ProductInfoVo(
							rs.getString("pcode"), 
	...
	public int getCountExist(String category, int order, String colorVal, String sizeVal, String priceVal, String div){
		Connection con=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		try {
			...
			System.out.println("ProductInfoDAO:getCountExist:"+se.getMessage());
			return -1;
		}finally {
			JdbcUtil.close(con,pstmt,rs);
		}
